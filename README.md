...
`apt-get install finger` | Adds a new user 
`apt-get install finger` | Nice program to check that the user is added sucessfully
`finger grader` | Checks that the new user was created 
...
`sudo nano /etc/ssh/sshd_config` | Open the configuration file
`Port 2200` |Change from Port 22 | 
`CTRL+O Enter CTRL+X` | Save and close
`sudo service ssh restart` | Restart the service for the changes to take effect
`ssh -i ~/.ssh/catalog grader@52.32.222.26 -p 2200` |If all is well need to make sure that we are connecting on port 2200 and not on default port 22

`sudo ufw deny 22` | Configure UFW to deny all connections on port 22 
...
[Default page][4] | Visit the following url to dispay the default page. By default Apache serves file from  /var/www/html directory
